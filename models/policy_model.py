...
class InstallmentClass(models.Model):
    ...
    # hishamCode
    bounce = fields.Integer(string="Collection Bounce", required=False,compute='claculateBoun' )
    check_date = fields.Boolean(string="",deafault=False  )
    hide_inst_button = fields.Boolean(string="",deafault=False  )
    ...
    def claculateBoun(self):
        print("sssssss")
        s = self.env['policy.broker'].search([('std_id', '=', self.policy_number)])
        if s.product_policy:
            if self.delv_insurer == True  :
                print("dddddd")
                date1= fields.Date.from_string(self.date)
                date2=date1 +relativedelta(months=1)
                date3=fields.Date.to_string(date2)
                print(date3)
                print(self.delv_date_ins)
                if date3 >= self.delv_date_ins:
                    print("fffff")
                    self.check_date = True
                    self.hide_inst_button=True
                    print(s.product_policy.product_bounce)
                    self.bounce = s.product_policy.product_bounce
        else:
            raise ValidationError('Your policy must have an product ,, please check your policy information')
    # hishamCode
    @api.multi
    def send_mail_template_policy(self):
        # Find the e-mail template
        self.ensure_one()
        ir_model_data = self.env['ir.model.data']
        template_id = self.env.ref('smart_policy.mail_temp')
        try:
            compose_form_id = ir_model_data.get_object_reference('mail', 'email_compose_message_wizard_form')[1]
        except ValueError:
            compose_form_id = False
    ...
    def create_invoice(self):
        ...
        bouns_bill = self.env['account.invoice'].create({
            'type': 'in_invoice',
            'partner_id': s.company.id,
            'insured_invoice': 'insurer_bill',
            'name': 'Insurer bouns for ' + str(s.company.name),
            'user_id': s.env.user.id,
            'insurance_id': s.id,
            'origin': s.policy_number,
            'insured_type': s.insurance_type,
            'insured_lOB': s.line_of_bussines.id,
            'insured_insurer': s.company.id,
            'insured_product': s.product_policy.id,
            'date_due': self.delv_date_ins,
            'invoice_line_ids': [(0, 0, {
                'name': str(s.line_of_bussines.line_of_business),
                'quantity': 1,
                'price_unit': self.bounce,
                'account_id': s.line_of_bussines.expense_account.id,
            })],
        })
        bouns_bill.action_invoice_open()
        self.hide_inst_button=False