...
class inhertResPartner(models.Model):
    ...
    # hishamCode
    opportunities_count = fields.Integer(compute='_compute_opportunities_count')
    ...
    def _compute_opportunities_count(self):
        if self.insurer_type == False:
            if self.agent == False :
                 self.opportunities_count = self.env['crm.lead'].search_count([('partner_id', '=', self.id)])
        else:
            if self.agent == True:
                 self.opportunities_count = self.env['crm.lead'].search_count([('user_id', '=', self.id)])
            else:
                if self.insurer_type == True:
                    self.opportunities_count = self.env['crm.lead'].search_count([('proposal_opp.Company', '=', self.id)])

    @api.multi
    def show_partner_opportunities(self):
        s='crm__lead_form_view'
        tree = self.env.ref('Smart_crm.ibs_crm_case_tree_view_oppor')
        form = self.env.ref('Smart_crm.crm__lead_form_view')
        return {
            'name': ('Opportunity'),
            'view_type': 'form',
            'view_mode': 'tree,form',
            ...
            'domain': ['|','|',('proposal_opp.Company', '=', self.id),('partner_id', '=', self.id),('user_id.partner_id', '=', self.id)]
        }
    @api.one
